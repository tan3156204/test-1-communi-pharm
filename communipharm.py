...
# ==========================================
st.set_page_config(page_title="PharmaSim V52.0 (Accounting Fixed)", layout="wide")
...
# Baseline Inputs (Reference from inputc1p1)
BASELINE_INPUTS = [
...
# Initial Financial State (CRITICAL FIX: Added AR and correct AP)
# Values estimated from Period 1 Start State
INIT_FINANCIALS = {
    ...
    "ar": [22000, 35000, 40000, 38000, 36000, 32000, 18000] # Estimated from sales lag
}
...
def init_game(n_stores):
    ...
    for i in range(n_stores):
        ...
        # Set default Purchases to sustain business
        default_inputs[14] = 40000.0 if ref_idx == 0 else 60000.0
        default_inputs[15] = 16000.0 if ref_idx == 0 else 80000.0
        # Set default AP Payment to previous AP (to prevent debt spiral)
        default_inputs[28] = float(INIT_FINANCIALS['ap'][ref_idx])
        
        default_inputs[16] = 1.0 if ref_idx != 0 else 0.0 # Store 1 has 0 pharm in inputc1p1
...
def run_period_simulation():
    ...
    for pid, p in st.session_state.players.items():
        ...
        # --- 4. EXPENSES ---
        weeks = 52.0 / env['periods_per_year'] # approx 8.66
        
        # Overtime Calculation (Fixed)
        # If Pharmacists = 0, assumes Manager covers or no Pharmacist cost (Store 1 case)
        ...
        if pharmacists > 0:
            ...
            base_pharm_cost = 0
            ot_pharm_cost = 0 # Manager salary covers it
        ...
        # --- 5. CASH FLOW (FIXED) ---
        # Inflows: Cash Sales (assumed 100% - Credit%) + Collection of OLD AR
        # Simplified: We assume current sales enter AR, and we collect OLD AR.
        # Or: Cash Sales part + AR Collection part.
        # Let's use: Cash In = (Collection of Prev AR) + (Cash Portion of Current Sales)
        # Assuming collection rate applies to TOTAL outstanding.
        
        # Standard Sim Logic:
        # Cash Receipts = (Beg AR + Credit Sales) * CollectionRate + Cash Sales
        # Simplified for this sim:
        # Collection Rate affects how much of "Total Receivables" comes in.
        
        receivables_pool = fin['ar'] + total_sales # Old AR + New Sales
        collection_rate = 0.95 - (max(0, env['pct_3rd_party'] - 40) * 0.005) # Approx 80-90%
        
        cash_in = receivables_pool * collection_rate
        new_ar = receivables_pool - cash_in
        
        # Outflows
        # You pay EXPENSES + A/P PAYMENT (for previous inventory) + LOAN PAYMENTS
        # Purchases go to AP, not Cash (unless COD, but here we use AP)
        ...
        if temp_ending_cash < 2500:
            ...
            # Interest reduces profit, usually paid next period, but here we book it now
        ...
        # AP Logic: Old AP + New Purchases - Paid Amount
...
# 6. MAIN APP
# ==========================================
st.sidebar.title("💊 PharmaSim V52 (Acc. Fix)")
